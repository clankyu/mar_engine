...
void draw_render_entity(Frame_Buffer *frame_buffer, Render_Entity e) {
    ...
    m4f scale = m4f_scale(e.scale);
    m4f rotation = m4f_rotate(e.rotation);
    m4f translation = m4f_translate(e.pos);
    m4f model = translation * rotation * scale;

    // world space to view space
    v3f from = engine.camera.pos;
    v3f direction = v3f_norm(v3f_lookat(engine.camera.rotation));
    v3f to = direction + from;
    ...
    m4f view = m4f_transpose(m4f_lookat(from, to, up));

    // view space to clip space
    m4f projection = m4f_perspective(
...
void clip_and_draw_triangle(Frame_Buffer *frame_buffer, Clip_Triangle triangle) {
    if (triangle_in_view_frustum(triangle)) {
        v4f v0_pos = triangle.v0.pos;
        v4f v1_pos = triangle.v1.pos;
        v4f v2_pos = triangle.v2.pos;
...
Clip_Triangle create_clip_triangle_mvp(Triangle triangle, m4f mvp) {
    ...
    v4f v0 = v3f_to_v4f(triangle.v0); v0.w = 1.0f;
    v4f v1 = v3f_to_v4f(triangle.v1); v1.w = 1.0f;
    v4f v2 = v3f_to_v4f(triangle.v2); v2.w = 1.0f;
...
Vertex4d vertex4d_lerp(Vertex4d a, Vertex4d b, f32 alpha) {
    Vertex4d result;
    result.pos = v4f_lerp(a.pos, b.pos, alpha);
