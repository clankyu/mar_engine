...
void execute_shader_pipeline(Shader_Pipeline *pipeline) {
    u64 vertex_count = pipeline->vertex_count; // todo: how get that? i think i set it when setting attributes
    
    for (u64 vertex_index = 0; vertex_index < vertex_count; ++vertex_index) {
        // who knows what else work we gotta do
        u8 *output_ptr = arena_push_struct(pipeline->gpu_arena, sizeof(V4), 1);
        pipeline->vertex_shader(pipeline, output_ptr, vertex_index);
    }
    
    // vertices are in clip space, so time to clip
    for (u64 triangle_index = 0; triangle_index < triangle_count; ++triangle_index) {
        V3u triangle_indices = get_triangle_indices(pipeline, triangle_index);
...
// note: super ugly code
void clip_and_draw_one_out(Shader_Pipeline *pipeline, u32 out_index, u32 in1_index, u32 in2_index) {
    V4 out = get_pos(pipeline, out_index); 
    V4 in1 = get_pos(pipeline, in1_index);
    V4 in2 = get_pos(pipeline, in2_index);
    ...
    clip_info1.alpha0_2 = 1.0f - alpha0; clip_info1.alpha2_0 = alpha0;
    ...
    memcpy(v0_ptr0, get_inputs_ptr(pipeline, indices0.v0), pattern_size);
    u8 *v1_ptr0 = arena_push(pipeline->clipping_arena, pattern_size);
    memcpy(v1_ptr0, get_inputs_ptr(pipeline, indices0.v1), pattern_size);
    u8 *v2_ptr0 = arena_push(pipeline->clipping_arena, pattern_size);
    memcpy(v2_ptr0, get_inputs_ptr(pipeline, indices0.v2), pattern_size);
    
    u8 *v0_ptr1 = arena_push(pipeline->clipping_arena, pattern_size);
    memcpy(v0_ptr1, get_inputs_ptr(pipeline, indices1.v0), pattern_size);
    u8 *v1_ptr1 = arena_push(pipeline->clipping_arena, pattern_size);
    memcpy(v1_ptr1, get_inputs_ptr(pipeline, indices1.v1), pattern_size);
    u8 *v2_ptr1 = arena_push(pipeline->clipping_arena, pattern_size);
    memcpy(v2_ptr1, get_inputs_ptr(pipeline, indices1.v2), pattern_size);
...
void clip_and_draw_two_out(Shader_Pipeline *pipeline, u32 out1_index, u32 out2_index, u32 in_index) {
    V4 out1 = get_pos(pipeline, out1_index); // todo: implement this lol
    V4 out2 = get_pos(pipeline, out2_index);
    V4 in = get_pos(pipeline, in_index);
    ...
    memcpy(v0_ptr, get_inputs_ptr(pipeline, indices.v0), pattern_size);
    
    u8 *v1_ptr = get_inputs_ptr(pipeline, in_index);
    
    u8 *v2_ptr = arena_push(pipeline->clipping_arena, pattern_size);
    memcpy(v2_ptr, get_inputs_ptr(pipeline, indices.v2), pattern_size);
...
// todo: fix this fucking function LOL or something i don't even know
...
f32 load_attribute_f32(Shader_Value_Array *attributes, u64 vertex_index, u64 attribute_index) {
    f32 result = 0.0f;
    result = *((f32*)attributes->items[attribute_index].data + vertex_index);
    
    return result;
}

V2 load_attribute_v2(Shader_Value_Array *attributes, u64 vertex_index, u64 attribute_index) {
    V2 result = {};
    result = *((V2*)attributes->items[attribute_index].data + vertex_index);
    
    return result;
}

V3 load_attribute_v3(Shader_Value_Array *attributes, u64 vertex_index, u64 attribute_index) {
    V3 result = {};
    result = *((V3*)attributes->items[attribute_index].data + vertex_index);
    
    return result;
}

V4 load_attribute_v4(Shader_Value_Array *attributes, u64 vertex_index, u64 attribute_index) {
    V3 result = {};
    result = *((V4*)attributes->items[attribute_index].data + vertex_index);
    
    return result;
}

M3 load_attribute_m3(Shader_Value_Array *attributes, u64 vertex_index, u64 attribute_index) {
    V3 result = {};
    result = *((M3*)attributes->items[attribute_index].data + vertex_index);
    
    return result;
}

M4 load_attribute_m4(Shader_Value_Array *attributes, u64 vertex_index, u64 attribute_index) {
    V3 result = {};
    result = *((M4*)attributes->items[attribute_index].data + vertex_index);
    
    return result;
}

// todo: check if this shit even works lol
f32 load_uniform_f32(Shader_Value_Array *uniforms, u64 uniform_index) {
    f32 result = 0.0f;
    result = *((f32*)uniforms->items[attribute_index].data);
    
    return result;
};

V2 load_uniform_v2(Shader_Value_Array *uniforms, u64 uniform_index) {
    V2 result = 0.0f;
    result = *((V2*)uniforms->items[attribute_index].data);
    
    return result;
};

V3 load_uniform_v3(Shader_Value_Array *uniforms, u64 uniform_index) {
    V3 result = 0.0f;
    result = *((V3*)uniforms->items[attribute_index].data);
    
    return result;
};

V4 load_uniform_v4(Shader_Value_Array *uniforms, u64 uniform_index) {
    V4 result = 0.0f;
    result = *((V4*)uniforms->items[attribute_index].data);
    
    return result;
};

M3 load_uniform_m3(Shader_Value_Array *uniforms, u64 uniform_index) {
    V2 result = 0.0f;
    result = *((M3*)uniforms->items[attribute_index].data);
    
    return result;
};

M4 load_uniform_m4(Shader_Value_Array *uniforms, u64 uniform_index) {
    M4 result = 0.0f;
    result = *((M4*)uniforms->items[attribute_index].data);
    
    return result;
};

// note: i think now that we've clipped we can just multiply by barycentric coordinates
f32 internal_in_smooth_f32(Shader_Value_Type_Array *output_pattern, u8 *v0_ptr, u8 *v1_ptr, u8 *v2_ptr, f32 v0_weight, f32 v1_weight, f32 v2_weight, u64 input_index) {
    f32 result = 0.0f; 
    f32 v0 = in_flat_f32(output_pattern, v0_ptr, input_index);
    f32 v1 = in_flat_f32(output_pattern, v1_ptr, input_index);
    f32 v2 = in_flat_f32(output_pattern, v2_ptr, input_index);
...
V2 internal_in_smooth_v2(Shader_Value_Type_Array *output_pattern, u8 *v0_ptr, u8 *v1_ptr, u8 *v2_ptr, f32 v0_weight, f32 v1_weight, f32 v2_weight, u64 input_index) {
    V2 result = {};
    V2 v0 = in_flat_v2(output_pattern, v0_ptr, input_index);
    V2 v1 = in_flat_v2(output_pattern, v1_ptr, input_index);
    V2 v2 = in_flat_v2(output_pattern, v2_ptr, input_index);    
...
V3 internal_in_smooth_v3(Shader_Value_Type_Array *output_pattern, u8 *v0_ptr, u8 *v1_ptr, u8 *v2_ptr, f32 v0_weight, f32 v1_weight, f32 v2_weight, u64 input_index) {
    V3 result = {};
    V3 v0 = in_flat_v3(output_pattern, v0_ptr, input_index);
    V3 v1 = in_flat_v3(output_pattern, v1_ptr, input_index);
    V3 v2 = in_flat_v3(output_pattern, v2_ptr, input_index);    
...
V4 internal_in_smooth_v4(Shader_Value_Type_Array *output_pattern, u8 *v0_ptr, u8 *v1_ptr, u8 *v2_ptr, f32 v0_weight, f32 v1_weight, f32 v2_weight, u64 input_index) {
    V4 result = {};
    V4 v0 = in_flat_v4(output_pattern, v0_ptr, input_index);
    V4 v1 = in_flat_v4(output_pattern, v1_ptr, input_index);
    V4 v2 = in_flat_v4(output_pattern, v2_ptr, input_index);    
...
f32 internal_in_flat_f32(Shader_Value_Type_Array *output_pattern, u8 *input_ptr, u64 input_index) {
    f32 result = 0.0f;
    u64 pattern_offset = get_pattern_offset(output_pattern, input_index);
...
void internal_fragment_shader_output(u8 *output_ptr, V4 color_result) {
    *(V4*)output_ptr = color_result;
}

Vertex_Shader diffuse(VERTEX_SHADER_PARAMETERS) {
    M4 model = load_uniform_m4(0);
    M4 view = load_uniform_m4(1);
    M4 projection = load_uniform_m4(2);
    
    V3 pos = load_attribute_v3(0);
    
    V4 pos = projection * view * model * create_v4(pos, 1.0f);
    
    output_pos(pos);
}

Fragment_Shader diffuse(FRAGMENT_SHADER_PARAMETERS) {
